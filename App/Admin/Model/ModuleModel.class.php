...
class ModuleModel extends Model{
    ...
    public function install($info) {
		...
		$_Sub_Dir = glob($path . 'Data/*.txt', GLOB_NOSORT);
        //判断是否存在安装的sql文件
        ...
        //移动样式文件夹
		$Dir = new \Dir();
		$Dir->copyDir('./App/'.$this->config['name'].'/Install/'.$this->config['name'].'','./Public/'.$this->config['name'],true);
    ...
    public function uninstall($module_info) {
		...
		$Dir = new \Dir();
		$Dir->unlinkDir('./Public/'.$module_info['name']);
		//更新数据库中的模块记录
	...
	public function cache(){
		...
		S('_OPEN_MOUDEL_LIST',null);
		
		$_OPEN_MOUDEL_LIST = $this->where(array('status'=>1,'disabled'=>1))->getField ( 'name',true );
		
		$_OPEN_MOUDEL_LIST[] = "Admin";
		
		S('_OPEN_MOUDEL_LIST', $_OPEN_MOUDEL_LIST);
	}
}